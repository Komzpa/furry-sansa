...
update %prefix%_osm_polygon t set "addr:street" = (select "name" from %prefix%_osm_line p where (p.tags?'highway') and name is not null and ST_DWithin(t.way, p.way, 400) order by ST_Distance(t.way, p.way) asc limit 1) where "addr:housenumber" is not null and "addr:street" is null;

update %prefix%_osm_point t set "addr:street" = (select "name" from %prefix%_osm_line p where (p.tags?'highway') and name is not null and ST_DWithin(t.way, p.way, 400) order by ST_Distance(t.way, p.way) asc limit 1) where "addr:housenumber" is not null and "addr:street" is null;
